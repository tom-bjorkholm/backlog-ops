...
def test_def_points_skipped() -> None:
    ...
    assert points.interpolate is False
    assert points.extrapolate is False


def test_def_points_table() -> None:
    """Test the guess table stores two levels and fills in between them.

    The two pre-filled rows are edited to level 1 with two story points
    and level 3 with eight, and both filling-in questions are left at
    ...
    config = backlog_ops_wizard(bridge(answers))
    points = config.default_story_points
    assert [(one.level, one.story_points) for one in points.levels] == \
        [(1, 2.0), (3, 8.0)]
    assert points.get_default_story_points(2) == 4.0
    assert points.get_default_story_points(4) == 16.0
...
def test_def_points_one_row() -> None:
    ...
    assert [(one.level, one.story_points) for one in points.levels] == \
        [(2, 5.0)]
    assert points.get_default_story_points(3) is None


def test_def_points_dup_level() -> None:
    """Test a level given twice is reported and the table is re-asked.

    Both rows are filled with level 2, which the check of the whole
    guess refuses, and the second row is then corrected.
    """
    dup = ['y', 'n', 'n', '1', '2', '5', ':+', '2', '9', '',
           '2', '3', '9', '']
...
def test_def_points_seeded() -> None:
    ...
    assert [(one.level, one.story_points) for one in points.levels] == \
        [(1, 2.0), (3, 8.0)]
    assert points.interpolate is True
