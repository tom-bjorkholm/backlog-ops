...
resources that record or refuse field updates, so the matching by name,
the map-based update payload, the empty-value no-op, the missing-name
policies (raise, ignore, add) and the never-mutated argument are checked
...
def test_update_existing(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert not result.ignored and not result.added and not result.failed
...
def test_empty_date_noop(monkeypatch: pytest.MonkeyPatch) -> None:
    """Test a release with no planned date leaves the Jira version alone."""
    client = _UpdClient(existing=['R1'])
    connections = _connections(monkeypatch, client)
...
def test_missing_add(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    client = _UpdClient(existing=['R1'])
    connections = _connections(monkeypatch, client)
    releases = [Release('R1'), Release('R2', date(2026, 2, 2))]
...
def test_format_updates() -> None:
    """Test the listing shows the four sections with their entries."""
    result = UpdatedReleasesInJira(
        updated=['R1'], ignored=['R2'], added=['R3'],
