...
"""Tests for the backlog operations driven from a backlog window."""
...
from backlogops_gui.backlog_dialogs import (
    DepOptions, ReleaseOrderOptions, StartChoice)
from backlogops_gui.format_dialogs import WriteOptions
...
def test_save_success(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    written: list[str] = []
    monkeypatch.setattr(backlog_actions, 'write_backlog', _writer(written))
...
def test_save_cancel_file(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(backlog_actions, 'choose_output_file', _no_file)
    monkeypatch.setattr(backlog_actions, 'write_backlog', _no_write)
    infos: list[tuple[str, str]] = []
...
def test_save_error(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(backlog_actions, 'choose_output_file', _out_csv)
    monkeypatch.setattr(backlog_actions, 'ask_write_options', _ok_options)
    monkeypatch.setattr(backlog_actions, 'write_backlog', _write_fail)
    errors: list[tuple[str, str]] = []
    infos: list[tuple[str, str]] = []
    save_backlog(_parent(), DATA, None, None, SINK, _record(errors),
                 _record(infos))
...
def test_save_cancel_options(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(backlog_actions, 'choose_output_file', _out_csv)
    monkeypatch.setattr(backlog_actions, 'ask_write_options', _no_options)
    monkeypatch.setattr(backlog_actions, 'write_backlog', _no_write)
    infos: list[tuple[str, str]] = []
    save_backlog(_parent(), DATA, None, None, SINK, _record([]),
                 _record(infos))
...
def test_order_keys_cancel(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(backlog_actions, 'ask_keys', lambda _p, _s: None)
    done: list[bool] = []
    data = _FakeData()
...
def test_order_deps_cancel(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(backlog_actions, 'ask_dep_options', lambda _p: None)
    done: list[bool] = []
    data = _FakeData()
...
def test_plan_dates_cancel(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(backlog_actions, 'ask_buffer_days', lambda _p: None)
    done: list[bool] = []
    data = _FakeData()
...
def test_order_keys_success(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(backlog_actions, 'ask_keys', lambda _p, _s: ['A', 'B'])
    done: list[bool] = []
    data = _FakeData()
    infos: list[tuple[str, str]] = []
    order_by_keys(PARENT, _as_data(data), SINK, _refresher(done), _record([]),
...
def test_order_keys_error(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(backlog_actions, 'ask_keys', lambda _p, _s: ['X'])
    done: list[bool] = []
...
def test_order_deps_success(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(backlog_actions, 'ask_dep_options', lambda _p: options)
    done: list[bool] = []
    data = _FakeData()
    infos: list[tuple[str, str]] = []
    order_by_deps(PARENT, _as_data(data), SINK, _refresher(done), _record([]),
...
def test_order_release_cancel(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(backlog_actions, 'ask_release_order', lambda _p: None)
    done: list[bool] = []
    data = _FakeData()
...
def test_order_release_plain(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(backlog_actions, 'ask_release_order',
...
def test_order_dates_cancel(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(backlog_actions, 'ask_date_order', lambda _p: None)
    done: list[bool] = []
    data = _FakeData()
...
def test_order_dates_planned(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(backlog_actions, 'ask_date_order', lambda _p: False)
    done: list[bool] = []
    data = _FakeData()
    infos: list[tuple[str, str]] = []
    order_dates(PARENT, _as_data(data), SINK, _refresher(done), _record([]),
...
def test_order_dates_est(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(backlog_actions, 'ask_date_order', lambda _p: True)
    done: list[bool] = []
    data = _FakeData()
    infos: list[tuple[str, str]] = []
    order_dates(PARENT, _as_data(data), SINK, _refresher(done), _record([]),
...
def test_adjust_success(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(backlog_actions, 'ask_buffer_days', lambda _p: 7)
    shown: list[tuple[str, str, object]] = []
    monkeypatch.setattr(backlog_actions, 'show_changes',
                        _changes_recorder(shown))
    done: list[bool] = []
...
def test_adjust_error(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(backlog_actions, 'ask_buffer_days', lambda _p: 5)
    monkeypatch.setattr(backlog_actions, 'show_changes', _no_write)
    done: list[bool] = []
...
def test_plan_dates_success(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(backlog_actions, 'ask_buffer_days', lambda _p: 3)
    shown: list[tuple[str, str, object]] = []
    monkeypatch.setattr(backlog_actions, 'show_changes',
                        _changes_recorder(shown))
    done: list[bool] = []
...
def test_save_changes_success(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(backlog_actions, 'choose_changes_output', _saver)
    written: list[str] = []
    infos: list[tuple[str, str]] = []
...
def test_save_changes_none(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(backlog_actions, 'choose_changes_output', _no_write)
    infos: list[tuple[str, str]] = []
...
def test_save_changes_cancel(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(backlog_actions, 'choose_changes_output', _no_file)
    infos: list[tuple[str, str]] = []
...
def test_show_changes(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def fake_list(_parent: object, title: str, text: str,
        ...
        return object()
    monkeypatch.setattr(backlog_actions, 'show_change_list', fake_list)
...
def test_extract_keys_success(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(backlog_actions, 'ask_levels', lambda _p: [1])
    monkeypatch.setattr(backlog_actions, 'choose_key_list_output',
                        lambda _p: 'keys.txt')
    written: list[tuple[list[str], str]] = []
    monkeypatch.setattr(backlog_actions, 'write_key_list',
                        _key_writer(written))
    infos: list[tuple[str, str]] = []
...
def test_extract_no_levels(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(backlog_actions, 'ask_levels', lambda _p: None)
    monkeypatch.setattr(backlog_actions, 'write_key_list', _no_write)
    infos: list[tuple[str, str]] = []
    extract_keys(PARENT, DATA, SINK, _record([]), _record(infos))
    assert not infos
...
def test_extract_error(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(backlog_actions, 'choose_key_list_output',
                        lambda _p: 'keys.txt')
    monkeypatch.setattr(backlog_actions, 'write_key_list', _key_write_fail)
    errors: list[tuple[str, str]] = []
