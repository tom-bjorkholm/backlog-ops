# Documentation

## User manual

To learn how to use backlogops from the CLI and the GUI (and which library
function is behind each action), start with the user manual:

- [Using backlogops](using_backlogops/README.md)

## API documentation

For more detailed code documentation, see the API documentation:

- [Library public API documentation](https://github.com/tom-bjorkholm/backlog-ops/blob/master/doc/backlogops_api.md)

- [Library protected API documentation](https://github.com/tom-bjorkholm/backlog-ops/blob/master/doc/backlogops_protected_api.md)

- [CLI public code documentation](https://github.com/tom-bjorkholm/backlog-ops/blob/master/doc/backlogops_cli.md)

- [CLI protected code documentation](https://github.com/tom-bjorkholm/backlog-ops/blob/master/doc/backlogops_protected_cli.md)

- [GUI public code documentation](https://github.com/tom-bjorkholm/backlog-ops/blob/master/doc/backlogops_gui.md)

- [GUI protected code documentaton](https://github.com/tom-bjorkholm/backlog-ops/blob/master/doc/backlogops_protected_gui.md)
