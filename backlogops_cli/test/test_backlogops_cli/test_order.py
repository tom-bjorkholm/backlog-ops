...
"""Tests for the backlogops_cli order command."""
...
from backlogops_cli import order
...
def test_in_command_list() -> None:
    ...
    assert 'order' in [name for name, _ in command_modules()]
...
def test_requires_args(args: list[str]) -> None:
    ...
    with pytest.raises(SystemExit):
        order.build_parser().parse_args(args)
...
def test_reorders_backlog(tmp_path: Path) -> None:
    ...
    assert order.main(['-i', str(source), '-k', str(key_file),
                       '-o', str(target)]) == 0
...
def test_missing_input(tmp_path: Path) -> None:
    ...
    key_file = tmp_path / 'keys.txt'
    key_file.write_text('E1\n', encoding='utf-8')
    assert order.main(['-i', str(tmp_path / 'no.csv'), '-k', str(key_file),
